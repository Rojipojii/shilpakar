...
<form method="post" action="dashboard.php">
        ...
        <input type="submit" value="Filter" class="right blue waves-effect waves-light btn">
        <?php
        if (!empty($category)) {
...
<li>
    <form method="post" action="dashboard.php">
        ...
        <select  class="browser-default" name="organizer" id="organizer">
...
<h2 align = "right">Top 10 Attendees:</h2>
    <thead>
        <tr>
            <th>Full Name</th>
    ...
    <tbody>
        <?php
        // Create an SQL query to get the top 10 attendees
        $sqlTopAttendees = "SELECT full_name, COUNT(*) AS total_events_attended
            FROM subscribers
        ...
        if ($resultTopAttendees->num_rows > 0) {
            while ($rowTopAttendees = $resultTopAttendees->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $rowTopAttendees["full_name"] . "</td>";
                echo "<td>" . $rowTopAttendees["total_events_attended"] . "</td>";
                echo "</tr>";
            }
        } else {
            echo "<tr><td colspan='2'>No top attendees found.</td></tr>";
        }
        ?>
    </tbody>
</table>
